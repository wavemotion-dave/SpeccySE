...
// ------------------------------------------------------------------------------------------------
// For the JR conditional instructions, the Cycle[] table builds in assuming the return will
// NOT be taken and so we must consume the additional cycles if the condition proves to be TRUE...
// ------------------------------------------------------------------------------------------------
case JR_NZ:   if(CPU.AF.B.l&Z_FLAG) {CPU.PC.W++;} else { M_JR; CPU.TStates+=5; } break; //12:435, 7:43
case JR_NC:   if(CPU.AF.B.l&C_FLAG) {CPU.PC.W++;} else { M_JR; CPU.TStates+=5; } break; //12:435, 7:43
case JR_Z:    if(CPU.AF.B.l&Z_FLAG) { M_JR; CPU.TStates+=5; } else {CPU.PC.W++;} break; //12:435, 7:43
case JR_C:    if(CPU.AF.B.l&C_FLAG) { M_JR; CPU.TStates+=5; } else {CPU.PC.W++;} break; //12:435, 7:43

case JP_NZ:   if(CPU.AF.B.l&Z_FLAG) CPU.PC.W+=2; else { M_JP; } break;  //10:433
case JP_NC:   if(CPU.AF.B.l&C_FLAG) CPU.PC.W+=2; else { M_JP; } break;  //10:433
case JP_PO:   if(CPU.AF.B.l&P_FLAG) CPU.PC.W+=2; else { M_JP; } break;  //10:433
case JP_P:    if(CPU.AF.B.l&S_FLAG) CPU.PC.W+=2; else { M_JP; } break;  //10:433
case JP_Z:    if(CPU.AF.B.l&Z_FLAG) { M_JP; } else CPU.PC.W+=2; break;  //10:433
case JP_C:    if(CPU.AF.B.l&C_FLAG) { M_JP; } else CPU.PC.W+=2; break;  //10:433
case JP_PE:   if(CPU.AF.B.l&P_FLAG) { M_JP; } else CPU.PC.W+=2; break;  //10:433
case JP_M:    if(CPU.AF.B.l&S_FLAG) { M_JP; } else CPU.PC.W+=2; break;  //10:433
...
// -----------------------------------------------------------------------------------------
case RET_NZ:  if(!(CPU.AF.B.l&Z_FLAG)) { M_RET; CPU.TStates+=6; } break; //11:533, 5:5
case RET_NC:  if(!(CPU.AF.B.l&C_FLAG)) { M_RET; CPU.TStates+=6; } break; //11:533, 5:5
case RET_PO:  if(!(CPU.AF.B.l&P_FLAG)) { M_RET; CPU.TStates+=6; } break; //11:533, 5:5
case RET_P:   if(!(CPU.AF.B.l&S_FLAG)) { M_RET; CPU.TStates+=6; } break; //11:533, 5:5
case RET_Z:   if(CPU.AF.B.l&Z_FLAG)    { M_RET; CPU.TStates+=6; } break; //11:533, 5:5
case RET_C:   if(CPU.AF.B.l&C_FLAG)    { M_RET; CPU.TStates+=6; } break; //11:533, 5:5
case RET_PE:  if(CPU.AF.B.l&P_FLAG)    { M_RET; CPU.TStates+=6; } break; //11:533, 5:5
case RET_M:   if(CPU.AF.B.l&S_FLAG)    { M_RET; CPU.TStates+=6; } break; //11:533, 5:5

case CALL_NZ: if(CPU.AF.B.l&Z_FLAG) CPU.PC.W+=2; else { M_CALL; CPU.TStates+=7; } break; //17:43433, 10:433
case CALL_NC: if(CPU.AF.B.l&C_FLAG) CPU.PC.W+=2; else { M_CALL; CPU.TStates+=7; } break; //17:43433, 10:433
case CALL_PO: if(CPU.AF.B.l&P_FLAG) CPU.PC.W+=2; else { M_CALL; CPU.TStates+=7; } break; //17:43433, 10:433
case CALL_P:  if(CPU.AF.B.l&S_FLAG) CPU.PC.W+=2; else { M_CALL; CPU.TStates+=7; } break; //17:43433, 10:433
case CALL_Z:  if(CPU.AF.B.l&Z_FLAG) { M_CALL; CPU.TStates+=7; } else CPU.PC.W+=2; break; //17:43433, 10:433
case CALL_C:  if(CPU.AF.B.l&C_FLAG) { M_CALL; CPU.TStates+=7; } else CPU.PC.W+=2; break; //17:43433, 10:433
case CALL_PE: if(CPU.AF.B.l&P_FLAG) { M_CALL; CPU.TStates+=7; } else CPU.PC.W+=2; break; //17:43433, 10:433
case CALL_M:  if(CPU.AF.B.l&S_FLAG) { M_CALL; CPU.TStates+=7; } else CPU.PC.W+=2; break; //17:43433, 10:433
...
case DEC_A:    
    ...
    break;
case DEC_xHL:  I=RdZ80(CPU.HL.W);M_DEC(I);INC_RW;WrZ80(CPU.HL.W,I);break;
...
case INC_xHL:  I=RdZ80(CPU.HL.W);M_INC(I);INC_RW;WrZ80(CPU.HL.W,I);break;
...
case RST00:    M_RST(0x0000);break; //11:533
case RST08:    M_RST(0x0008);break; //11:533
case RST10:    M_RST(0x0010);break; //11:533
case RST18:    M_RST(0x0018);break; //11:533
case RST20:    M_RST(0x0020);break; //11:533
case RST28:    M_RST(0x0028);break; //11:533
case RST30:    M_RST(0x0030);break; //11:533
case RST38:    M_RST(0x0038);break; //11:533

case PUSH_BC:  M_PUSH(BC);break; //11:533
case PUSH_DE:  M_PUSH(DE);break; //11:533
case PUSH_HL:  M_PUSH(HL);break; //11:533
case PUSH_AF:  M_PUSH(AF);break; //11:533
...
case DJNZ:   // 13:535, 8:53
  ...
  if(--CPU.BC.B.h) { M_JR; CPU.TStates+=5; } else { CPU.PC.W++;} break;

case JP:   M_JP;break;   //10:433
case JR:   M_JR;break;   //12:435
...
case EX_HL_xSP: //19:43435
  J.B.l=RdZ80(CPU.SP.W);WrZ80(CPU.SP.W++,CPU.HL.B.l);INC_RW;
